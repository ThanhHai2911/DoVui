...
class _GameCompleteScreenState extends State<GameCompleteScreen>
  ...
  bool _isVip = false;
  bool _isVipLoaded = false; // chờ load xong mới render

  @override
  void initState() {
    super.initState();
    _loadVipFromFirebase(); // ✅ query thẳng Firebase
  ...
  // ✅ Query thẳng Firestore — không tin widget.isVip hay AdsService()
    ...
    try {
      ...
      if (userId != null && userId.isNotEmpty) {
        ...
        if (mounted) {
          ...
          // ✅ Cập nhật luôn AdsService để đồng bộ toàn app
      ...
      // Fallback: dùng widget.isVip nếu Firebase lỗi
      if (mounted) {
        setState(() {
          _isVip = widget.isVip;
          _isVipLoaded = true;
  ...
  // Thay SingleChildScrollView + Column bằng cấu trúc Stack/Column có Spacer
